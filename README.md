...
- **新增連續顯示模式**：工具列 / 顯示選單可在 **單頁** 與 **連續捲動** 間切換。
  連續模式垂直堆疊所有頁面、延遲渲染（只算視窗附近頁），捲動位置與側欄、頁碼雙向連動；
  標註與選取工具在連續模式下會對應到游標所在頁。
- **修正打包 exe 的嚴重問題（程序炸彈）**：v0.2 的 onefile exe 在缺少
  `pdfium.dll`（`pypdfium2_raw`）時，會因舊的 import fallback 用自身重新啟動而**無限自我複製**。
  - 打包腳本補上 `--collect-all pypdfium2_raw`，確保 `pdfium.dll` 一定被包進 exe。
  - 程式加入凍結環境防護：打包後若缺元件，只跳錯誤訊息並結束，**絕不自我重啟**。
- **修正縮圖側欄版面**：拉入很寬的圖片（或大尺寸頁面）時，縮圖會撐爆側欄、把其他頁的縮圖
  擠到看不見。改為每張縮圖統一框進固定方框（寬高皆不超框），任何比例都不會破版。

> ⚠️ **強烈建議 v0.2 使用者更新到 v0.3**：v0.2 的 `WinPreview.exe` 在部分環境下可能觸發
> 上述自我複製問題；v0.3 已修正並加入防護。
